...
import {
  ...
  Smile, Check, CheckCheck, Sparkles, Heart, Volume2
} from "lucide-react";
...
function Avatar({ url, username, size = "md", online = false }: {
  ...
  return (
    <div className="relative shrink-0">
      ...
      {online && (
        <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-white shadow" />
      )}
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <motion.h2
          ...
          className="text-2xl md:text-3xl font-black text-pink-600 uppercase tracking-tight mt-4"
        >
          ✨ New Gift!
        </motion.h2>
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 0.7 }}
          className="text-pink-400 font-bold mt-1"
        >
          {gift.name}
        </motion.p>
      </motion.div>
...
function MessageBubble({ m, isMe, onReply, onDelete }: {
  ...
  const isReply = m.content.includes("⤵️");
  ...
  return (
    ...
    >
      {!isMe && (
        <div className="self-end mb-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
      )}

      <div className={`relative max-w-[78%] md:max-w-[65%]`}>
        {/* Reply preview */}
        {replyPreview && (
          <div className={`mb-1 px-3 py-1.5 rounded-xl text-[10px] italic truncate border-l-2 ${
            isMe ? "bg-pink-400/30 border-pink-300 text-pink-100" : "bg-pink-50 border-pink-300 text-pink-500"
          }`}>
        ...
        {/* Bubble */}
        <div className={`relative px-4 py-2.5 rounded-[1.2rem] shadow-sm ${
          isMe
            ? "bg-gradient-to-br from-pink-500 to-rose-500 text-white rounded-br-md"
            : "bg-white border border-pink-100 text-zinc-800 rounded-bl-md"
        }`}>
          {isGift ? (
            <div className="text-center py-1 px-2">
              <img src={m.content.split("|")[0].replace("[GIFT]:", "")} className="w-14 h-14 mx-auto object-contain" />
          ...
          {/* Tail */}
          {isMe ? (
            <div className="absolute -bottom-0.5 -right-1 w-3 h-3 bg-rose-500 clip-tail-right" />
          ) : (
            <div className="absolute -bottom-0.5 -left-1 w-3 h-3 bg-white clip-tail-left" />
          )}
        </div>
        ...
        <AnimatePresence>
          {showActions && (
            ...
            >
              <button onClick={onReply} className="p-1 text-pink-400 hover:text-pink-600 transition-colors" title="Reply">
                <Reply size={12} />
              </button>
              <div className="w-px h-3 bg-pink-100" />
              <button onClick={onDelete} className="p-1 text-red-300 hover:text-red-500 transition-colors" title="Delete">
                <Trash2 size={12} />
              </button>
            </motion.div>
          )}
...
export default function DirectChatPage() {
  ...
  const fetchRecentChats = useCallback(async (userId: string) => {
    ...
    if (data) {
      const partnerMap = new Map<string, string>(); // partnerId → last message
      data.forEach(m => {
  ...
  return (
    <div className="fixed inset-0 flex font-sans overflow-hidden" style={{
      background: "linear-gradient(135deg, #FFF0F6 0%, #FDF2F8 50%, #FFF5F7 100%)"
    }}>

      {/* Global gift animation */}
      <AnimatePresence>
        {activeGiftAnim && (
          <GiftAnimation gift={activeGiftAnim} onDone={() => setActiveGiftAnim(null)} />
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className="px-5 pt-6 pb-4 space-y-4">
          <div className="flex items-center justify-between">
            <Link href="/app" className="flex items-center gap-2 group">
              <div className="p-2 bg-gradient-to-br from-pink-500 to-rose-500 rounded-xl text-white shadow-md shadow-pink-200 group-hover:shadow-pink-300 transition-shadow">
                <ArrowLeft size={16} />
              </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-1">
          {displayList.length === 0 && searchQuery.length >= 2 && (
            <div className="text-center py-8 text-pink-200 text-xs font-bold uppercase tracking-widest">
              No users found
            </div>
          )}
              ...
              <div className="flex-1 min-w-0">
                ...
                {u.lastMessage && (
                  <p className="text-[10px] text-zinc-400 truncate mt-0.5">
                    {u.lastMessage.startsWith("[GIFT]:") ? "🎁 Sent a gift" : u.lastMessage}
                  </p>
                )}
              </div>
              {activePartner?.id === u.id && (
      ...
      <main className={`${!activePartner ? "hidden md:flex" : "flex"} flex-1 flex-col h-full relative`}>
        ...
        {activePartner ? (
          <>
            ...
            >
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="md:hidden p-2 text-pink-400 hover:text-pink-600 hover:bg-pink-50 rounded-xl transition-all"
                >
                  <ChevronLeft size={22} />
                </button>
                <Avatar url={activePartner.avatar_url} username={activePartner.username} online={activePartner.is_live} size="md" />
                <div>
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 text-zinc-400 hover:text-zinc-700 hover:bg-pink-50 rounded-xl transition-all"
                >
                  ...
                  {showOptions && (
                    ...
                    >
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-4 py-3 text-red-500 hover:bg-red-50 text-xs font-bold transition-colors"
                      >
            ...
            >
              {/* Date separator — just once for demo */}
              {messages.length > 0 && (
              ...
              <AnimatePresence>
                {replyTo && (
                  ...
                  >
                    ...
                    <button onClick={() => setReplyTo(null)} className="text-pink-300 hover:text-pink-500 ml-2 shrink-0">
                      <X size={13} />
                    </button>
                  </motion.div>
                )}
              ...
              <div className="flex gap-2 overflow-x-auto pb-0.5 no-scrollbar">
                {quickEmojis.map(e => (
                  <button
                    ...
                    className="text-lg hover:scale-125 active:scale-95 transition-transform shrink-0"
                  >
                    {e}
                  </button>
                ))}
              ...
              {showGiftModal && (
                ...
                >
                  <div className="px-5 py-4 border-b border-pink-100 flex items-center justify-between shrink-0">
                    <div>
                      <h2 className="font-black text-lg text-zinc-900 tracking-tight">
                        Send a Gift <span className="text-pink-500">🎁</span>
                      </h2>
                      <p className="text-[10px] text-zinc-400 font-medium mt-0.5">
                        to @{activePartner.username}
                      </p>
                    </div>
                    <button
                      onClick={() => { setShowGiftModal(false); setGiftError(false); }}
                      className="p-2 bg-pink-50 hover:bg-pink-100 rounded-xl text-pink-400 transition-all"
                    >
                      <X size={18} />
                    </button>
                  </div>
                        ...
                        {giftTypes.map((gt, i) => (
                          ...
                          >
                            {sendingGift === gt.id ? (
                              ...
                              <img src={gt.image_url} className="w-14 h-14 object-contain group-hover:scale-110 transition-transform" />
                            )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
            ...
            <p className="text-sm text-zinc-400 max-w-xs leading-relaxed">
              Pick a conversation from the left or search for someone to start messaging.
            </p>
          </div>
        )}
      </main>
