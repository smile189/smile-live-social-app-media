/*
this the wallet component, where users can see their coins and diamonds balance, and also have the option to withdraw or exchange their diamonds for coins. 
The design is inspired by modern finance apps, with a focus on clarity and ease of use. The page is divided into two main sections: 
one for coins and one for diamonds, each with its own card displaying the balance and relevant actions. 
The user can switch between the two sections using a toggle at the top. The coins section allows users to purchase more coins,
 while the diamonds section provides options for withdrawing earnings or exchanging them for coins. The page also includes 
 a profile header at the top, showing the user's avatar, username, and role, along with a back button to return to the main app. Overall, 
 this component aims to provide a seamless and engaging experience for users managing their in-app currency.
...
import { 
  ...
  ShieldCheck, ArrowUpCircle, User
} from "lucide-react";
...
export default function WalletPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Luăm datele din ambele tabele simultan
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#FDF8F9]">
    ...
    <div className="min-h-screen bg-[#FDF8F9] pb-10">
      ...
      <div className="bg-white px-6 pt-12 pb-6 rounded-b-[3rem] shadow-sm mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-3xl border-2 border-pink-100 overflow-hidden bg-pink-50">
            {profile?.avatar_url ? (
              <img src={profile.avatar_url} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-pink-300"><User /></div>
            )}
          </div>
          <div>
            <h2 className="font-black text-zinc-800 text-lg">@{profile?.username}</h2>
            <div className="flex gap-2 mt-1">
              <span className="bg-pink-100 text-pink-600 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">
                {profile?.role || 'User'}
              </span>
              {profile?.agency_id && (
                <span className="bg-blue-100 text-blue-600 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">Agency Partner</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main className="max-w-md mx-auto px-6">
        {/* Switcher */}
        <div className="flex bg-pink-100/50 p-1.5 rounded-[2rem] mb-6">
          {(["coins", "diamonds"] as const).map((tab) => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-white text-pink-600 shadow-sm" : "text-pink-400"}`}
            >
              {tab}
            </button>
          ))}
            ...
            <motion.div key="coins" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[100, 500, 1000, 5000].map((amount) => (
                  <button key={amount} className="bg-white border border-pink-50 p-6 rounded-[2.2rem] flex flex-col items-center gap-1 hover:border-pink-300 transition-all shadow-sm active:scale-95">
                    <Zap size={20} className="text-yellow-500 fill-yellow-500 mb-1" />
                    <span className="font-black text-zinc-800 text-lg">{amount}</span>
                    <span className="text-[10px] font-bold text-pink-400">€{(amount / 100).toFixed(2)}</span>
                  </button>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div key="diamonds" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-fuchsia-600 to-purple-600 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-5xl font-black tracking-tighter">0</h2> {/* Momentan 0 până adăugăm coloana diamonds */}
                </div>
              </div>
                ...
                <button className="w-full bg-white border border-purple-100 py-5 rounded-[2rem] flex items-center justify-between px-6 shadow-sm group hover:border-purple-300 transition-all">
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-widest">Bank / PayPal</p>
                    </div>
                  </div>
                ...
                <button className="w-full bg-white border border-purple-100 py-5 rounded-[2rem] flex items-center justify-between px-6 shadow-sm group hover:border-purple-300 transition-all">
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-widest">0% Fee • Instant</p>
                    </div>
                  </div>
                  <ArrowUpCircle className="text-zinc-200 group-hover:text-blue-500 transition-all rotate-90" />
                </button>
              </div>
